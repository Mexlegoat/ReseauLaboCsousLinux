 ...
 while(1)
 {
  ...
  switch(Choix)
  {
   ...
   case '4': 
             ...
             scanf("%d", &ref);
             printf("\n");
             res = RechercheHV("VehiculesHV", ref, &UnRecord); // Donner à la variable res un nombre retourné par la fonction
             if (res == 1) // Si la recherche a réussi
             {
                AfficheEnteteVehiculeHV(); // Afficher Entete
                AfficheVehiculeHV(&UnRecord); // Afficher le vehicule recherché: &UnRecord est actualisé par la fonction pour afficher le vehicule
             }
             else if (res == 0) // Si la recherche n'a pas réussi (La reference saisie ne se trouve pas dans la structure)
             {
                fprintf(stderr, "La Reference ne se trouve pas dans le fichier\n");
             }
             break ;
   case '6': ListingFacturesHV("FactureHV") ;
	     break ;
   case '7': AProposServeurHV("Puissance","MELLOUKI FILALI Issam","MELLOUKI FILALI Imad") ;
             break ;
